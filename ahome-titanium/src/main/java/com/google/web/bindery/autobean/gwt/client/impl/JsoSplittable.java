 ...
 */
@GwtScriptOnly
public final class JsoSplittable extends JavaScriptObject implements
		Splittable, HasSplittable {
	 ...
	 * objects to the output. The test for function objects is for old versions
	 * of Safari.
	 */
	...
	private native void assign0(Splittable parent, String index,
			Splittable value) /*-{
								parent[index] = value;
								}-*/;
	...
	/**
	 * The test for {@code $H} removes the key in the emitted JSON, however
	 * making a similar test for {@code __reified} causes the key to be emitted
	 * with an explicit {@code null} value.
	 */
