...
public class StackTraceCreator {
	...
	/**
	 * Maximum # of frames to look for {@link Throwable#fillInStackTrace()} in
	 * the generated stack trace. This is just a safe guard just in case if
	 * {@code fillInStackTrace} doesn't show up in the stack trace for some
	 * reason.
	 */
	...
	/**
	 * Replacement for class or file names that cannot be extracted from a
	 * stack.
	 */
	...
	/**
	 * This legacy {@link Collector} simply crawls
	 * <code>arguments.callee.caller</code> for browsers that doesn't support
	 * {@code Error.stack} property.
	 */
	static class CollectorLegacy extends Collector {
		...
		public native void collect(Object t, Object thrownIgnored) /*-{
			...
			if (arguments.callee.caller) {
				...
				while (callee) {
					var name = @com.google.gwt.core.client.impl.StackTraceCreator::getFunctionName(*)(callee);
					t.fnStack.push(name);
	...
	static final class CollectorEmulated extends Collector {
		...
		public native void collect(Object t, Object jsThrownIgnored) /*-{
			...
			for (var i = 0; i < $stackDepth; i++) {
				...
				var fn = $stack[i];
				var name = fn
						? @com.google.gwt.core.client.impl.StackTraceCreator::getFunctionName(*)(fn)
						: @com.google.gwt.core.client.impl.StackTraceCreator::ANONYMOUS;
		...
		public StackTraceElement[] getStackTrace(Object t) {
			...
			StackTraceElement[] stackTrace = new StackTraceElement[stack
					.length()];
			for (int i = 0; i < stackTrace.length; i++) {
				...
				stackTrace[i] = new StackTraceElement(UNKNOWN, name, fileName,
						lineNumber);
			}
			return stackTrace;
		}
	...
	static class CollectorModern extends Collector {
		...
		public StackTraceElement[] getStackTrace(Object t) {
			...
			// Chrome & IE10+ contains the error msg as the first line of stack
			// (iOS, Firefox doesn't).
		...
		/**
		 * Parses a stack trace line from the browser and returns a new
		 * {@link StackTraceElement} constructed with the extracted data.
		 */
		private StackTraceElement parse(String stString) {
			...
			if (index == -1) {
				...
				if (index == -1) {
					// No bracketed items nor '@' found, hence no function name
					// available
					location = toReturn;
					toReturn = "";
				} else {
			...
			if (lastColonIndex != -1 && endFileUrlIndex != -1) {
				...
				line = parseInt(location.substring(endFileUrlIndex + 1,
						lastColonIndex));
		...
		protected StackTraceElement createSte(String fileName, String method,
				int line, int col) {
	...
	static class CollectorModernNoSourceMap extends CollectorModern {
		...
		protected StackTraceElement createSte(String fileName, String method,
				int line, int col) {
			return new StackTraceElement(UNKNOWN, method, fileName,
					LINE_NUMBER_UNKNOWN);
		}
	}
	...
	/**
	 * Collect necessary information to construct stack trace trace later in
	 * time.
	 */
	...
	private static StackTraceElement[] dropInternalFrames(
		...
		final String dropFrameUntilFnName = Impl
				.getNameOf("@com.google.gwt.core.client.impl.StackTraceCreator::captureStackTrace(*)");

		int numberOfFrameToSearch = Math.min(stackTrace.length,
				DROP_FRAME_LIMIT);
	...
	static {
		...
		collector = (c instanceof CollectorModern && enforceLegacy) ? new CollectorLegacy()
				: c;
	}
    ...
    return fn.name || (fn.name = @com.google.gwt.core.client.impl.StackTraceCreator::extractFunctionName(*)(fn.toString()));
  }-*/;
