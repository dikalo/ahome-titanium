...
public abstract class DOMImpl {
	 ...
	 * 
	 * <p>
	 * Note: you should be aware that this uses JavaScript rounding and thus does NOT provide the same semantics as <code>int b = (int) someDouble;</code>. In particular, if x is
	 * outside the range [-2^31,2^31), then toInt32(x) would return a value equivalent to x modulo 2^32, whereas (int) x would evaluate to either MIN_INT or MAX_INT.
	 */
	...
	@Deprecated
	public abstract NativeEvent createKeyEvent(Document doc, String type, boolean canBubble, boolean cancelable, boolean ctrlKey, boolean altKey, boolean shiftKey,
	...
	public abstract NativeEvent createMouseEvent(Document doc, String type, boolean canBubble, boolean cancelable, int detail, int screenX, int screenY, int clientX, int clientY,
			boolean ctrlKey, boolean altKey, boolean shiftKey, boolean metaKey, int button, Element relatedTarget);
